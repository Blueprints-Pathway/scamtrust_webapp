...
function CustomerTransaction() {
	...
	const validate = (values) => {
		...
		if (!values.pin1 || !values.pin2 || !values.pin3 || !values.pin4) {
			...
			values.pin1 !== values.pinCheck1 ||
			values.pin2 !== values.pinCheck2 ||
			values.pin3 !== values.pinCheck3 ||
			values.pin4 !== values.pinCheck4
		) {
			...
			<div className="password-view-left">
				...
				<div className="cardContainerTransaction">
					...
					<div className="form transaction">
						<div className="field">
							<h4 className="form-textTransaction">
								To set up your pin, input your 4-digit code and confirm it
								below.
							</h4>
						</div>
						...
						<form autoComplete="off">
							...
							{submitted && error.pinCheckError1 && (
								<span className="error tranc">
									<FontAwesomeIcon
										className="exIconTrans"
										icon={faCircleExclamation}
									/>
									{error.pinCheckError1}
								</span>
							)}

							<div className="input-fieldPin">
								<label className="form-textTranc2">
							...
							{submitted && error.pinCheckError2 && (
								<span className="error tranc2">
									<FontAwesomeIcon
										className="exIconTrans"
										icon={faCircleExclamation}
									/>
									{error.pinCheckError2}
								</span>
							)}
						</form>
						...
						submitted &&
						!error.pinCheckError1 ? (
							<Link to="/CustomerSecurityQuestion">
								{" "}
								<button className="Transaction-button yes">
									{" "}
									Set security question{" "}
								</button>{" "}
							</Link>
						) : (
							<button onClick={handleSubmit} className="Transaction-button yes">
								{" "}
								Set security question{" "}
							</button>
						)}
					</div>
					<div className="field-bottom-transaction">
						{" "}
						Secured by{" "}
						<img
