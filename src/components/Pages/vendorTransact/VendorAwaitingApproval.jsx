...
const VendorAwaitingApproval = ({ showAcceptedHandler }) => {
	const [awaiting, setAwaiting] = useState();
	const [modalIsOpen, setModalIsOpen] = useState(false);

	const id = awaiting?.transaction_id;
	function openModal() {
		setModalIsOpen(true);
	}
	...
	const Awaiting = localStorage?.getItem("idAwaiting");
	...
	useEffect(() => {
		(async () => {
			try {
				...
				setAwaiting(data?.data?.data);
				console.log(data?.data.data, "awaiting data");
			} catch (error) {
				console.log(error, "error from vendor transaction");
			}
		})();
	...
	const accepted = async () => {
		...
		try {
			...
			console.log(error, "errorss from awaiting");
		}
	};
    const customStyles = {
		content: {
					...
					<div className="flex flex-wrap items-center justify-between md:mx-5">
						<div>
							...
							<p className="text-sm lg:text-2xl font-bold text-[#262466] my-1 lg:my-4">
								{" "}
								{awaiting?.customer?.name || awaiting?.customer?.username}{" "}
							</p>
						</div>
					...
					<div className="border-[1px] border-[#95999D] rounded-2xl lg:mt-2">
						...
						<div className="flex-column items-center justify-center mt-7 px-5 lg:px-28">
							<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
								...
								<p className="text-[#8D9296]">{awaiting?.product_name}</p>
							</div>
							<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
								<p className="font-medium">Product amount</p>
								<p className="text-[#8D9296]">₦{awaiting?.total_amount}</p>
							</div>
							<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
								<p className="font-medium">Quantity</p>
								<p className="text-[#8D9296]">{awaiting?.quantity}</p>
							</div>
							<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
								<p className="font-medium">Due date</p>
								<p className="text-[#8D9296]">{awaiting?.due_date}</p>
							</div>
