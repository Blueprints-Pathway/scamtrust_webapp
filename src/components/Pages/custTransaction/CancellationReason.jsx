...
const CancellationReason = ({ showOngoingHandler }) => {
	...
	const cancelled = async () => {
		try {
			const API_URL = `${process.env.REACT_APP_BASE_URL}/transaction/vendor/cancel/${onGoing}`;
			const config = {
				headers: {
		...
		<div>
			<Layout>
				<div className="border-[1px] bg-white rounded-xl lg:rounded-3xl mt-16 ml-4 pt-5 lg:pt-12 pb-7 lg:pb-20 px-8 lg:mx-12 lg:px-28">
					...
					>
						<BsArrowLeft className="text-xl lg:text-2xl mr-1" />
						Back
					</p>
					<h1 className="text-xl lg:text-4xl lg:font-bold font-semibold mt-5 lg:mt-8">
					...
					<form action="submit" className="mt-8 lg:mt-16">
						<div className="flex items-center justify-start text-center border-[1px] border-[#002257] py-3 pl-7 lg:pl-16">
							...
							<label
								for="cancelReason1"
								className="text-lg text-[#002257] lg:text-3xl pl-2 lg:pl-5 lg:py-2"
							>
						...
						<div className="flex-column items-center justify-center py-10 px-10 md:px-16 lg:py-32 lg:px-48">
							...
							<div>
								<h1 className="text-[#002257] text-center text-xl lg:text-3xl font-light w-60 lg:w-96 leading-6 mb-6 lg:mb-8">
									TR - {onGoing}
									<span className="text-[#95999D]">
										<br />
										was cancelled. Reason being,{reason}
									</span>
								</h1>
							</div>
