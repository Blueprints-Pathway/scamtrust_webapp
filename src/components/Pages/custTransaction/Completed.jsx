...
const Completed = ({ showCompletedHandler }) => {
	...
	useEffect(() => {
		(async () => {
			try {
				const API_URL = `https://scamtrust.herokuapp.com/api/v1/transaction/view/${complete}`;
				const config = {
					headers: {
	...
	return (
		<Layout>
			{isLoading ? (
				<img
					src={logo}
					className="flex   m-auto transition-timing-function: cubic-bezier(0.4, 0, 1, 1) animate-bounce"
					alt=""
				/>
			) : (
					...
					<div className="bg-[#fff] rounded-3xl md:w-[53rem] w-[22rem] mr-9 lg:ml-[-185px] mt-[-80px] md:mt-[-72px] lg:w-[90rem] lg:mt-[-75px] lg:pb-16 py-10 pt-12 lg:pt-12 md:pt-8 md:px-12 px-5">
						...
						<div className="flex flex-wrap items-center justify-between md:mx-5">
							...
							<div className="md:ml-[-30px] lg:ml-[-15rem]">
								...
								<p className="text-sm lg:text-2xl mt-1 lg:my-4 font-normal text-[#262466]">
									{moment(completed?.updated_at).format("DD MM YYYY hh:mm")}
								</p>
							</div>
							<div>
								{/* S T A G E S */}
								<div className="flex items-center justify-center mt-6 mx-[55px] md:mx-0">
									<div className="flex-column items-center justify-center">
										<div className="flex items-center justify-center">
											...
											<div className="w-16 lg:w-24 border-[#3AB75D] border-[2px]"></div>
										</div>
						...
						<div className="border-[1px] border-[#95999D] rounded-2xl lg:mt-2">
							...
							<div className="flex-column items-center justify-center mt-7 px-5 lg:px-28">
								<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
									...
									<p className="text-[#8D9296]">{completed?.product_name}</p>
								</div>
								<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
									<p className="font-medium">Product amount</p>
									<p className="text-[#8D9296]">₦{completed?.total_amount}</p>
								</div>
								<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
									<p className="font-medium">Quantity</p>
									<p className="text-[#8D9296]">{completed?.quantity}</p>
								</div>
								<div className="flex items-center justify-between text-base lg:text-2xl my-7 lg:my-10">
									<p className="font-medium">Due date</p>
									<p className="text-[#8D9296]">
										{moment(completed?.created_at).format("DD/MM/YYYY")}
									</p>
								</div>
