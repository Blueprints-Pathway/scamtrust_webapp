...
const Ongoing = () => {
	...
	let transaction = transactions.ongoingTransactions.find(
		(transaction) => transaction.id == param.id
	);

	if (usertype === "VENDOR") {
		transaction = vendorTransactions.allTransactions.find(
			(transaction) => transaction.id == param.id
		);
	}
	...
	useEffect(() => {
		if (transactions.isAcceptTransactionSuccessful) {
			...
			navigate(`/ongoing-transaction/:id/ratting`);
		}
					...
					<div className={classes["top"]}>
						...
						<div className={classes["top-left"]}>
							<div className={classes["left-1"]}>
								<p className={classes["top-left-id"]}>
									ID - {transaction.transaction_id}
								</p>
								<p className={classes["top-left-vendor"]}>
									{transaction?.vendor?.name || transaction?.customer?.username}{" "}
								</p>
							</div>
							{/* TOP MIDDLE */}
							<div className={classes["top-middle"]}>
								...
								<p className={classes["top-middle-date"]}>
